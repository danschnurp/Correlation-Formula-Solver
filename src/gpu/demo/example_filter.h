...
struct ExampleFilter {
 ...
 public: // data
  ...
  mutable vk::CommandBuffer
      cmdBuffer; ///< commands recorded here, once command buffer is submitted to a queue those commands get executed
 ...
 private: // helpers
  static auto createInstance(const std::vector<const char *> layers, const std::vector<const char *> extensions
  ) -> vk::Instance;
  ...
  static auto createPipelineLayout(const vk::Device &device, const vk::DescriptorSetLayout &dscLayout
  ) -> vk::PipelineLayout;

  static auto createComputePipeline(const vk::Device &device,
                                    const vk::ShaderModule &shader,
                                    const vk::PipelineLayout &pipeLayout,
                                    const vk::PipelineCache &cache
  ) -> vk::Pipeline;

  static auto createDescriptorSet(const vk::Device &device,
